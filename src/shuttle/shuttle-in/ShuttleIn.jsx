...
export default function ShuttleIn({ location: { search }, match: { url } }) {
    const [commonCx, shuttleCx, shuttleInCx] = useStyle(commonInputStyles, shuttleInput, shuttleInStyles)
    ...
    return <div className={shuttleInCx('container')}>
        <div className={shuttleCx('input-arrow', 'with-icon')}>
            <img className={shuttleCx('icon')} src={'https://via.placeholder.com/50'}></img>
            <input readOnly
            ...
            <Link to={{
                pathname: '/token',
                search: `?next=${url}`
            }} ><img className={shuttleCx('arrow')} src={arrow}></img></Link>
        </div>
